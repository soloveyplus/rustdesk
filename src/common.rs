...
pub async fn do_check_software_update() -> hbb_common::ResultType<()> {
    // ПОЛНОСТЬЮ ОТКЛЮЧАЕМ ПРОВЕРКУ ОБНОВЛЕНИЙ
    // Не выполняем никаких HTTP-запросов, не сравниваем версии,
    // просто сбрасываем URL и ничего не делаем.
    
    // Сбрасываем URL обновления, чтобы уведомление не появлялось
    *SOFTWARE_UPDATE_URL.lock().unwrap() = "".to_string();
    
    // Завершаем функцию успешно, ничего не делая
    Ok(())
}
